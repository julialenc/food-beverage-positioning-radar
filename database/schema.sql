-- schema.sql
-- Auto-generated by pipeline/exports/export_schema.py — do not edit by hand.
-- Generated: 2026-09-08 07:57:26
...
CREATE TABLE axis_range_config (
    ...
    computed_at             TEXT
, beverage_view_segment TEXT NOT NULL DEFAULT 'all', p03 REAL, p97 REAL, n_below_p03 INTEGER, n_above_p97 INTEGER, n_trimmed INTEGER);

CREATE TABLE category_region_averages (
    snapshot                 TEXT NOT NULL,
...
CREATE TABLE market_chart_bands (
    id                              INTEGER PRIMARY KEY AUTOINCREMENT,
    snapshot                        TEXT NOT NULL,
    region_code                     TEXT NOT NULL,
    category                        TEXT NOT NULL,
...
, off_brands_raw TEXT, off_brand_tokens TEXT, legacy_primary_brand TEXT, brand_entity_raw TEXT, brand_entity_source TEXT, normalized_brand TEXT, brand_alias_source TEXT, brand_alias_review_status TEXT, energy_kcal_off_raw REAL, fat_100g_off_raw REAL, saturated_fat_100g_off_raw REAL, carbs_100g_off_raw REAL, sugars_100g_off_raw REAL, fiber_100g_off_raw REAL, protein_100g_off_raw REAL, salt_100g_off_raw REAL, nutrition_quality_status TEXT, outlier_type TEXT, include_in_product_table INTEGER, include_in_aggregates INTEGER, include_in_charts INTEGER, nutrition_quality_reason TEXT, energy_kcal_missing INTEGER, fat_100g_missing INTEGER, saturated_fat_100g_missing INTEGER, carbs_100g_missing INTEGER, sugars_100g_missing INTEGER, fiber_100g_missing INTEGER, protein_100g_missing INTEGER, salt_100g_missing INTEGER, resolved_company TEXT, brand_family TEXT, company_ownership_resolution_status TEXT, company_mapping_source TEXT, warning_flag INTEGER, warning_types TEXT, warning_summary TEXT);
...
CREATE INDEX idx_analysis_claim_source ON product_analysis(claim_source);
...
CREATE INDEX idx_category_region_averages_snapshot ON category_region_averages(snapshot);

CREATE INDEX idx_market_chart_bands_lookup
    ON market_chart_bands(snapshot, region_code, category, barcode);
...
CREATE INDEX idx_products_ingested_at ON products(ingested_at);

CREATE INDEX idx_products_modified ON products(last_modified_t);

CREATE INDEX idx_products_normalized_brand ON products(normalized_brand);

CREATE INDEX idx_products_nova ON products(nova_group);

CREATE INDEX idx_products_primary_brand ON products(primary_brand);

CREATE INDEX idx_products_resolved_company ON products(resolved_company);

CREATE INDEX idx_products_snapshot_category ON products(ingested_at, query_category);

CREATE INDEX idx_products_snapshot_category_brand ON products(ingested_at, query_category, normalized_brand, primary_brand);

CREATE INDEX idx_products_snapshot_category_company ON products(ingested_at, query_category, resolved_company);

CREATE INDEX idx_products_snapshot_company ON products(ingested_at, resolved_company);

CREATE INDEX idx_products_snapshot_nutriscore ON products(ingested_at, nutriscore_grade);

CREATE INDEX idx_products_snapshot_product_name ON products(ingested_at, product_name);
